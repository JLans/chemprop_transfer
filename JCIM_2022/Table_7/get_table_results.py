...
if __name__ == '__main__':
    ...
    combined_dir = './combined_10M+all'
    directoryCs = ['direct_model', './ani_all', './ani_energy', './ani_energy+fcs', './ani_fcs'
               , './ani_energy+vib', './ani_vib', './ani_energy+sym'
               , './ani_sym', './ani_energy+MOI', './ani_MOI']
    data = {'directoryCs': directoryCs
    ...
    for directory in data['directoryCs']:
        test_path = os.path.join(combined_dir, 'test_full.csv')
        ...
        for fold in fold_list:
            ...
            fold_in = os.path.join(combined_dir, fold)
            train_path = os.path.join(fold_in, 'train_full.csv')
            train_data = read_csv(train_path)
            val_path = os.path.join(fold_in, 'val_full.csv')
            val_data = read_csv(val_path)
            test_preds_path = os.path.join(fold_out, 'test_preds.csv')
            train_preds_path = os.path.join(fold_out, 'train_preds.csv')
            val_preds_path = os.path.join(fold_out, 'val_preds.csv')
