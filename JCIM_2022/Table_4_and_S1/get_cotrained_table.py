...
if __name__ == '__main__':
    target = 'heat_of_formation (kcal/mol)'
    target2 = 'electronic_energy (Hartree)'
    final_df = DataFrame(data={'epochs':[], 'split_type':[]
                                ...
                                , 'ensemble_rmse':[], 'ensemble_rmse_se':[],'energy':[],'energy_se':[]})
    epochs=['10', '50', '100']
    seeds=['1', '2', '3']
    fold_list = ['fold_' + str(i) for i in range(3)]
    ...
    final_df.sort_values(by=['split_type', 'epochs'], inplace=True)
    print('saving data')
    final_df.to_csv('./Casey_cotrained_results.csv', index=False)
