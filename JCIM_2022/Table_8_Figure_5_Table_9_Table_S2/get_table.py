...
if __name__ == '__main__':
    target = 'log10h50(exp)'
    target2 = 'energy'
    final_df = DataFrame(data={'train_r2':[], 'train_r2_se':[]
    ...
    fold_list = ['fold_' + str(i) for i in range(5)] #5
    #Do not change below this line
    seed_dirs = [os.path.join('./best_model','seed_'+seed) for seed in seeds]
    sub_frame = DataFrame(data={})
    sub_data_list = [[], [], [], [], [], [], [], [], []]
    for seed_dir in seed_dirs:
        test_path = os.path.join(seed_dir, 'test_full.csv')
        ensemble_path = os.path.join(seed_dir, 'ensemble_preds.csv')
    ...
    for index, sub_list in enumerate(sub_data_list):
        column_avg = final_df.columns[2*index]
        column_std = final_df.columns[2*index+1]
